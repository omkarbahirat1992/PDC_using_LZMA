...
{
    FILE *fp_in2, *fp_in_backup;
    int init_flag=0;                          //Flag to decide whether to re_initilaze the input file pointer "fp_in" for next distance or not
    struct node* temp = hash_arr[key_idx].next;
    unsigned int temp_len = 0;
    char cur_ch;
    pair.len = 0;

    fseek(fp_in,-1,SEEK_CUR); //setting initial current file pointer
    fp_in_backup = fp_in;
    while(temp){

        //Re-initializing input pointer for next distance
        //fp_in =  fp_in_backup;
        if(init_flag)
            fseek(fp_in,-(pair.len+1),SEEK_CUR);
        ...
        fseek(fp_in2,temp->dist,SEEK_SET);   //setting file pointer at current distance
    ...
    count+=pair.len;
    write_record();
    fseek(fp_in,-1,SEEK_CUR);       //Setting fp_in at appropriat position, because of "while(1)" it got 1 position ahead
